...
public class INSERemoteImpl extends UnicastRemoteObject implements RemoteInterface{
	...
	public INSERemoteImpl() throws RemoteException {
		super();
	...
	public synchronized String addCourse(String courseID, String semester,int capacity) throws RemoteException{
		...
		//INSE advisor only can add INSE courses.
			...
			if(semester.equals(semname[i])) {
				...
				if(enrollist.get(courseID+semester) != null) {
					...
					while (name.hasNext()) {
						...
						//Remote modify
						if(n.contains("SOEN")) {
							reply = UDPrequest(7778, "removeCourse." + n + "." + courseID + "." + semester);
						...
						if(n.contains("COMP")) {
							reply = UDPrequest(5556, "removeCourse." + n + "." + courseID + "." + semester);
							//Check the reply status.
							if(reply.contains("success") == false) {
	...
	public synchronized String listCourseAvl(String semester) throws RemoteException{
		...
		for(int i = 0; i < 3; i++) {
			...
			if(semester.equals(semname[i])) {
				...
				reply1 = UDPrequest(7778, "listCourseAvl."+semname[i]);
				ServerLogging.getLogMessage(localHostName, UserID + "SOENServer reply: " + reply1);
				//UDP request for courses to COMP server.
				reply2 = UDPrequest(5556, "listCourseAvl."+semname[i]);
				ServerLogging.getLogMessage(localHostName, UserID + " COMPServer reply: " + reply2);
				HashMap<String, Integer> sem = courseRecords.get(semname[i]);
				if(sem == null) {
					ServerLogging.getLogMessage(localHostName, UserID + " INSE have no course"+"\n"+reply1+"\n"+reply2);
					return "INSE have no course"+"\n"+reply1+"\n"+reply2;
				}
				ServerLogging.getLogMessage(localHostName, UserID + " " + sem.toString()+"\n"+reply1+"\n"+reply2);
				return sem.toString()+"\n"+reply1+"\n"+reply2;
			}
		}
		ServerLogging.getLogMessage(localHostName, UserID + " List course available failed");
	...
	public synchronized String enrolCourse(String studentID, String courseID, String semester) throws RemoteException {
		...
		if(courseID.contains("SOEN")) {
			reply = UDPrequest(7778, "enrolCourse." + semester + "." + courseID + "." + studentID);
			//Check the reply status.
		...
		if(courseID.contains("COMP")) {
			reply = UDPrequest(5556, "enrolCourse." + semester + "." + courseID + "." + studentID);
			//Check the reply status.
			if(reply.contains("success")) {
				//Modify department limit.
				if(deptlimit.get(studentID) == null) {
	...
	public synchronized String dropCourse(String studentID, String courseID) throws RemoteException {
		...
		if(courseID.contains(localHostName)) {
			...
			for(int i = 0; i < 3; i++) {
				...
				ServerLogging.getLogMessage(localHostName, UserID + " " + studentID +" not contain in " + courseID+semname[i]);
			}
		...
		//Remote drop course
		if(courseID.contains("SOEN")) {
			reply = UDPrequest(7778, "dropCourse." + studentID + "." + courseID);
			//Check the reply status.
			if(reply.contains("success")) {
		...
		if(courseID.contains("COMP")) {
			reply = UDPrequest(5556, "dropCourse." + studentID + "." + courseID);
			//Check the reply status.
			if(reply.contains("success")) {
	...
	public synchronized String getClassSche(String studentID) throws RemoteException {
		...
		ServerLogging.getLogMessage(localHostName, UserID + " " + Sche.toString());
	...
	public void UDPreceiver(int port) {
		...
		try {
			...
			while (true) {
				...
				//
				//arg[0] = method    arg[1] = studentID    arg[2] = courseID

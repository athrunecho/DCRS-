...
public class COMPRemoteImpl extends UnicastRemoteObject implements RemoteInterface{
	private static final String localHostName = "COMP";
	...
	public COMPRemoteImpl() throws RemoteException {
		super();
			...
			if(semester.equals(semname[i])) {
				...
				if(enrollist.get(courseID+semester) != null) {
					...
					while (name.hasNext()) {
						...
						//Remote modify
						if(n.contains("SOEN")) {
							reply = UDPrequest(7777, "removeCourse." + n + "." + courseID + "." + semester);
					...
					if(n.contains("INSE")) {
						reply = UDPrequest(6666, "removeCourse." + n + "." + courseID + "." + semester);
						//Check the reply status.
						if(reply.contains("success") == false) {
	...
	public synchronized String enrolCourse(String studentID, String courseID, String semester) throws RemoteException {
		...
		if(courseID.contains("SOEN")) {
			reply = UDPrequest(7777, "enrolCourse." + semester + "." + courseID + "." + studentID);
			//Check the reply status.
		...
		if(courseID.contains("INSE")) {
			reply = UDPrequest(6666, "enrolCourse." + semester + "." + courseID + "." + studentID);
			//Check the reply status.
			if(reply.contains("success")) {
				//Modify department limit.
				if(deptlimit.get(studentID) == null) {
	...
	public synchronized String dropCourse(String studentID, String courseID) throws RemoteException {
		...
		//Remote drop course
		if(courseID.contains("SOEN")) {
			reply = UDPrequest(7777, "dropCourse." + studentID + "." + courseID);
			//Check the reply status.
			if(reply.contains("success")) {
		...
		if(courseID.contains("INSE")) {
			reply = UDPrequest(6666, "dropCourse." + studentID + "." + courseID);
			//Check the reply status.
			if(reply.contains("success")) {
